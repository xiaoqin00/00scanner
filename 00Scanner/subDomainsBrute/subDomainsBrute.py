...
class SubNameBrute:
    def __init__(self, target, options):
        ...
        if options['output']:
            ...
            outfile = target + '.txt' if not options['ful_scan'] else target + '_full.txt'
                ...
                try:
                    ...
                    if answers:
                        ...
                        if (not self.ignore_intranet) or (not SubNameBrute.is_intranet(answers[0].address)):
                            ...
                            self.msg_queue.put('status')
                            try:
                                sqlAdd='insert into subdomain(subdomain,ip)VALUES ("%s","%s")'%(str(cur_sub_domain.ljust(30)),str(ips))
...
def main(target):
    ...
    try:
        ...
        cursor=db.cursor()
        sqlDelete='delete from subdomain'
        cursor.execute(sqlDelete)
        db.commit()
    except Exception,e:
        print '1',e
    # try:
    #     os.remove('./../result/subDomain.txt')
    # except Exception,e:
    #     print e

    options={'full_scan': False, 'i': False, 'threads': 200, 'file': 'subnames.txt', 'output': 'result.txt'}
    print options['threads']
    d = SubNameBrute(target, options)
    d.run()
    d.outfile.flush()
    d.outfile.close()
    db.close()
