...
def log(scan_type,host,port,info=''):
    ...
    if scan_type == 'portscan':
        ...
        try:
            sqlAdd='insert into weakpass(target,scan_type,info)VALUES ("%s","%s","%s")'%(target,scan_type,info)
            cursor.execute(sqlAdd)
            db.commit()
        except Exception,e:
            print 'sqlAdd--error!',e
        # f=open('./../result/weakPass.txt','w')
        # f.write(log)
        # f.close()
        print log
...
def main(target):
    ...
    try:
        ...
        print 'sqlDelete--error!',e

    # try:
    #     os.remove('./../result/weakPass.txt')
    # except Exception,e:
    #     print e
    global TIMEOUT
    global I
