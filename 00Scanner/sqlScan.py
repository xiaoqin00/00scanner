...
import difflib, httplib, itertools, optparse, random, re, urllib, urllib2, urlparse,os
...
def main(url):
    ...
    cursor=db.cursor()
    sqlDelete='delete from sqlScan'
    cursor.execute(sqlDelete)
    db.commit()
    # except Exception,e:
    #     print 'sqlDlete--error!',e

    print 'sqlScan run!'
    init_options()
    # result = scan_page(url if url.startswith("http") else "http://%s" % url)
    result=scan_page(url)
    sqlAdd='insert into sqlScan(sqlScan)VALUES ("%s")'%url
    if result:
        try:
            cursor.execute(sqlAdd)
            db.commit()
        except Exception,e:
            print 'sqlScan--open error!',e
    db.close()
    return result
